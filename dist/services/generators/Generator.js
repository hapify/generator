...
let GeneratorService = class GeneratorService {
            ...
            for (const template of templates) {
                if (template.input === __1.TemplateInput.One) {
                    for (const model of models) {
                        ...
                        output.push(yield this._one(template, models, model));
                    }
                }
                else {
                    output.push(yield this._all(template, models));
                }
            }
            return output;
        });
     ...
     * @private
     */
    _explicitAllModels(models) {
        return models.map((mod) => this._explicitModel(models, mod));
    }
};
